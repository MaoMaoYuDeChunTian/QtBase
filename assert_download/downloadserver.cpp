...
DownloadServer::DownloadServer()
    ...
    , m_file(nullptr)
{
    m_saveFilePath = QCoreApplication::applicationDirPath()+"/assets.zip";
    downloadFileAsync(QUrl("http://ideavr.top/download/avatar-assets/assets.zip"),m_saveFilePath);
}

void DownloadServer::RequestData(QUrl url, const QString &saveFilePath)
{
    if (!url.isValid())
        return;

    QString _saveFilePath = QDir::cleanPath(saveFilePath);
...
void DownloadServer::downloadFileAsync(QUrl url, const QString &saveFilePath)
{
    RequestData(url, saveFilePath);
    if (!m_reply)
        return;
